...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function _mkdir($path, $name) {
		$path = $path.DIRECTORY_SEPARATOR.$name;
                echo $path;
               try{
                    file_put_contents(getcwd() . "/error.log", "mkdir: $path \n\r", FILE_APPEND);
                   array_push($this->error, "Mkdir: $path");
		if (mkdir($path)) {
			//@chmod($path, $this->options['dirMode']);
			return $path;
		}
               }catch(Exception $e){
                   file_put_contents(getcwd() . "/error.log", $e->getTraceAsString(), FILE_APPEND);
               } catch(Error $err){
                      file_put_contents(getcwd() . "/error.log", $err->getTraceAsString(), FILE_APPEND);
               }
