...
class elFinder {
	...
	protected function mkdir($args) {
		...
		if (($volume = $this->volume($target)) == false) {
			return array('error' => $this->error(self::ERROR_MKDIR , $name . " - " .  $target . " [0]", self::ERROR_TRGDIR_NOT_FOUND, '#'.$target));
		}
                
               $dir = false;
               $errmsg = "";
                $path = $this->params['roots'][0]['path'] .DIRECTORY_SEPARATOR.$name;
               try{
                  
                  $dir = mkdir($path);//$volume->mkdir($path, $name);
               } catch (Exception $ex) {
		...
		return ($dir) == false
			? array('error' => $this->error(self::ERROR_MKDIR, $name . " - " . $target . " -> " . getcwd() . " trace: " . $errmsg . " path: {{" . $path . "}} [1]", $volume->error()))
			: array('added' => array($path));
	}
