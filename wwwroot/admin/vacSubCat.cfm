		...
		<div class="conIndent">
	    	<div class="row-fluid">
	    		<div id="prodCon" class="span12 well">
					<a href="vacSubCatFunctions.cfm" class="quoteBtn">Add Subcategory</a>
	    			<h1>Subcategories</h1>
		    				...
		    				<ul id="#cat.c_url#" class="sortable">
								...
								<cfif cat.s_switch eq 1>
									<li id="#cat.c_id#_#cat.s_id#" class="caseBox">
										<h4 class="pull-left"><a href="vacSubCatFunctions.cfm?subCat=#cat.s_id#" title="Edit">#cat.s_title#</a></h4>
										<a href="##" class="delete icon-trash pull-right" rel="#cat.s_id#" title="Delete"></a>
										<a href="vacSubCatFunctions.cfm?subCat=#cat.s_id#" class="edit icon-edit pull-right" title="Edit"></a>
									</li>
								</cfif>
			    				</cfoutput>
		    				</ul>
	...
	$(document).ready(function() {
		...
		$('#sortMessage').on('click', 'a.btnLink', function() {
			...
			$.ajax({
				...
				url: 'update.cfm',
				data: {
					section: 'vacSubCat', <cfoutput>catList: '#catList#', <cfloop from="1" to="#listLen(catList)#" index="i">act#i#: #listGetAt(actList, i)#, </cfloop><cfloop from="1" to="#listLen(catList)#" index="j">in#j#: #listGetAt(inList, j)#<cfif j lt listLen(inList)>, </cfif></cfloop></cfoutput>
				},
		...
		$('.delete').click(function() {
			...
			var destination = 'delete.cfm?section=vacSubCat&subCat=' + subCat;
